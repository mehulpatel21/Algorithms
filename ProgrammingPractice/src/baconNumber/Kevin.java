...
public final class Kevin {
    ...
    private static void solveBacon() {
        Queue<Vertex<String>> myQueue = new LinkedList<Vertex<String>>();
        graph.label(bacon, bacon);
        myQueue.add(bacon);
        ...
        while (!myQueue.isEmpty()) {
            ...
            if (current == actor) {
                while (graph.label(actor) != bacon) {
                    System.out.println(actor.get());
                    actor = (Vertex<String>) graph.label(actor);
                }
                if (actor != bacon) {
                    System.out.println(actor.get());
                }
            ...
            for (Edge<String> e : graph.outgoing(current)) {
                Vertex<String> end = graph.to(e);
                if (graph.label(end) == null) {
                    myQueue.add(end);
                    graph.label(end, current);
                }
            }
        }
        System.out.println("Actor has no path to bacon");
        System.exit(0);
    }
